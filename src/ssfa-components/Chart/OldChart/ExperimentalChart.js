...
const ChartComponent = ({ varX, dataSet }) => {
  ...
  useEffect(() => {
    ...
    _.forEach(dataSet, (value, key) => {
      if (varX.includes(key)) data.push({ Country: key, Value: value });
    });
    ...
    const mouseover = function (event, d) {
      const value = d.Value;
      tooltip.html(value).style("position", "absolute").style("opacity", 1);
    };
      ...
      <Table>
        <TableHead>
          <TableRow style={{ backgroundColor: "#2196f3" }}>
            <TableCell align="center" style={{ color: "ghostWhite" }}>
              {varName}
            </TableCell>
            <TableCell align="center" style={{ color: "ghostWhite" }}>
              {valueName}
...
const BasicSelector = ({ selectionList, selection, setSelection, label }) => {
  ...
  return (
    <FormControl sx={{ m: 1, minWidth: 300, maxWidth: 900, padding: "1rem" }}>
      ...
      <Select value={selection} label={label} onChange={handleChange}>
...
function countData(filterList, filter, target, inclusive = []) {
  ...
  filterList.forEach((item) => {
    if (inclusive.length > 0) {
      let substitution = "";
      inclusive.forEach((inclusiveItem) => {
        substitution = item.includes(inclusiveItem.trim().toLowerCase())
          ? inclusiveItem.trim().toLowerCase()
          : substitution;
      });
  ...
  rows.forEach((row) => {
    row.c[position[filter]].v.split(";").forEach((item) => {
      ...
      let key =
        substitution.length > 0 ? substitution : item.trim().toLowerCase();

      let values =
        row.c[position[target]] !== (undefined || null)
          ? row.c[position[target]].v.split(";")
          : ["NA"];
...
export default function ExperimentalChart({
  ...
  return (
    <div>
      <ChartComponent dataSet={dataSet} varX={varNames} />
      <div
        style={{
          maxWidth: 900,
          margin: "30px auto",
