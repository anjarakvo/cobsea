...
export function countryNameFormatter(name) {
  let countryName = "";
  if (name === "Brunei") countryName = "Brunei Darussalam";
  else if (name === "South Korea") countryName = "Republic of Korea";
...
export function getAuthorCount(country) {
  ...
  rows.forEach((row) => {
    if (
      row.c[position["Location/Territory studied"]] !== null &&
      row.c[position["Author(s)"]] !== null
    )
      if (row.c[position["Location/Territory studied"]].v.trim() === country) {
        ...
        list.forEach((raw_author) => {
          ...
          if (isNaN(result[author])) result[author] = 1;
          else result[author] += 1;
        });
      }
  });
